...
class BinanceClient:
    def __init__(self, base_urls: tuple[str, ...]):
        ...
        self.session.headers.update({"User-Agent": "binance-8h-50pct-research/7.0"})
...
def download_archive(url: str, destination: Path) -> bool:
    response = requests.get(url, timeout=180, stream=True, headers={"User-Agent": "binance-8h-50pct-research/7.0"})
    ...
    checksum = requests.get(
        f"{url}.CHECKSUM",
        timeout=60,
        headers={"User-Agent": "binance-8h-50pct-research/7.0"},
    )
