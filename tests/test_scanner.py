...
class FakeBinance:
    ...
    def klines(self, symbol, interval, start_ms, end_ms):
        ...
        for minute in range(480):
            ...
            high = 126 if minute == 479 else 101
    ...
    def aggregate_trades(self, symbol, start_ms, end_ms, max_pages=500):
        ...
        if start_ms == crossing and end_ms - start_ms <= 60_000:
            return ([{"a": 2, "p": "125", "q": "1", "f": 2, "l": 2, "T": crossing + 1_000, "m": False, "M": True}], False)
        if start_ms == crossing + 1_000:
            # Exit liquidity exists below the +25% threshold. This must still pass
            # because V7 saleability proves executability, not price persistence.
            return ([{"a": 3, "p": "110", "q": "5", "f": 3, "l": 3, "T": start_ms + 2_000, "m": True, "M": True}], False)
        return ([], False)
...
def test_candidate_records_saleable_event_without_price_persistence():
    ...
    assert event["seller_taker_notional_any_price"] == 550.0
    assert event["seller_taker_notional_at_or_above"] == 0.0
    assert event["minimum_exit_vwap"] == pytest.approx(110.0)
